...
//const APIkey = "75ad6260ae4b6886ec9d2263e1800843";
...
function actions() {
  postIt().then(updateUI);
}

// Get data from API
const getAPI = async (zip) => {
  const openWeather = await fetch(
    `${APIbase}?zip=${zip}&units=imperial&APPID=${APIkey}`,
    {
      body: JSON.stringify(),
    }
  );
...
const updateUI = async () => {
  ...
  try {
    ...
    document.getElementById("date").innerHTML = `Date: ${
      everything[everything.length - 1].date
    }`;
    document.getElementById("temp").innerHTML = `Temperature: ${
      everything[everything.length - 1].weather
    }`;
    document.getElementById("content").innerHTML = `Feelings: ${
      everything[everything.length - 1].feelings
    }`;
  } catch (error) {
    console.log("error", error);
  }
